(() => {
  ...
  self.onmessage = async (e) => {
    ...
    if (type === "run") {
      try {
        const result = runHledger(e.data.journal, e.data.command, e.data.args);
  ...
  function runHledger(journalContent, command, args) {
    if (!cachedModule) {
    ...
    const fds = [
      ...
      new PreopenDirectory("/", /* @__PURE__ */ new Map([["journal.hledger", new File(journalBytes)]]))
    ];
    const wasi = new WASI(["hledger", command, "-f", "/journal.hledger", ...args], [], fds, { debug: false });
